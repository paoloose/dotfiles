...
def main(args: List[str]) -> Optional['ResultDict']:
    ...
    try:
        args, _rest = parse_args(args[1:], lambda: GRAB_ARGS)
        tty = open(os.ctermid())
...
def handle_result(
        args: List[str], result: 'ResultDict', target_window_id: WindowId, boss: Boss
    ) -> None:

    if not 'copy' in result or not (to_copy := result['copy'].strip()):
        return

    set_clipboard_string(to_copy)
